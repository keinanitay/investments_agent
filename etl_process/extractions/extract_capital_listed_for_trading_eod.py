"""
Extract capital listed for trading end of day from TASE API
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("extract_capital_listed_for_trading_eod.log"),
...
# API configuration
CAPITAL_LISTED_FOR_TRADING_EOD_ENDPOINT = f"{API_BASE_URL}/indices-parameters-updates/capital-listed-for-trading-end-of-day"

# MongoDB collection names
STG_COLLECTION = "stg_capital_listed_for_trading_eod"
CORE_COLLECTION = "core_capital_listed_for_trading_eod"


class CapitalListedForTradingEodExtractor:
    """
    Extract capital listed for trading end of day from TASE API
    """
    ...
    def __init__(self):
        ...
        logger.info("Capital Listed For Trading EOD: %s", CAPITAL_LISTED_FOR_TRADING_EOD_ENDPOINT)
    ...
    def extract_for_date(self, trade_date: date) -> Tuple[int, int, int]:
        """
        Extract capital listed for trading end of day for specific date
        Args:
        ...
        try:
            ...
            url = f"{CAPITAL_LISTED_FOR_TRADING_EOD_ENDPOINT}/{trade_date.year}/{trade_date.month}/{trade_date.day}"
            ...
            # Extract data
            capital_listed_for_trading = data.get('capitalListedForTrading', {})
            results = capital_listed_for_trading.get('result', [])
    ...
    def _create_document(self, record: Dict, extraction_date: date, trade_date: date) -> Dict:
        ...
        return {
            ...
            'ians_change': safe_int(record.get('iansChange')),
            'listed_capital': safe_int(record.get('listedCapital')),
            'capital_change': safe_int(record.get('capitalChange')),
    ...
    def merge_to_core(self) -> Tuple[int, int]:
        ...
        """
        logger.info("Merging capital_listed_for_trading_eod from staging to core...")
        ...
        try:
            ...
            for stg_doc in stg_docs:
                ...
                logger.info(f"Merged capital_listed_for_trading_eod: {inserted} inserted, {updated} updated")

                return (inserted, updated)

            except Exception as e:
                logger.error(f"Error merging capital_listed_for_trading_eod to core: {e}")
                raise


if __name__ == "__main__":
    """Standalone run - extract yesterday's data"""
    extractor = CapitalListedForTradingEodExtractor()
