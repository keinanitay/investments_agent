"""
Extract trading code list from TASE API
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("extract_trading_code_list.log"),
...
# API configuration
TRADING_CODE_LIST_ENDPOINT = f"{API_BASE_URL}/basic-securities/trading-code-list"

# MongoDB collection names
STG_COLLECTION = "stg_trading_code_list"
CORE_COLLECTION = "core_trading_code_list"


class TradingCodeListExtractor:
    """
    Extract trading code list from TASE API
    """
    ...
    def __init__(self):
        ...
        logger.info("Trading Code List: %s", TRADING_CODE_LIST_ENDPOINT)
        logger.info("Connected to API and MongoDB")
    
    def _create_indexes(self):
        """Create indexes for better query performance"""
        try:
            self.stg_collection.create_index([("list_type_id", 1)], unique=True)
            
            self.core_collection.create_index([("list_type_id", 1)], unique=True)
    ...
    def extract_data(self) -> Tuple[int, int, int]:
        """
        Extract trading code list data (uses today's date)
    ...
    def extract_for_date(self, extraction_date: date) -> Tuple[int, int, int]:
        """
        Extract trading code list data for specific date
        Args:
            extraction_date: Date to extract data for
        Returns: (saved, updated, skipped)
        """
        logger.info(f"Extracting trading code list data for {extraction_date}...")
    ...
    def _extract_and_save(self, extraction_date: date) -> Tuple[int, int, int]:
        """
        Extract and save trading code list
        Args:
        ...
        try:
            response = self.session.get(TRADING_CODE_LIST_ENDPOINT, timeout=30)
            ...
            if not results:
                logger.warning("No trading codes found")
                return 0, 0, 0
            
            logger.info(f"Found {len(results)} trading codes")
            ...
            logger.error(f"Error extracting trading code list: {e}")
            return 0, 0, 0
    ...
    def _save_batch(self, records: List[Dict], extraction_date: date) -> Tuple[int, int, int]:
        ...
        for record in records:
            try:
                # Extract list_type_id
                list_type_id = record.get('listTypeId')
                
                if not list_type_id:
                    logger.error(f"No list_type_id: {record}")
                    skipped += 1
                    continue
                ...
                # Upsert document
                filter_dict = {'list_type_id': list_type_id}
    ...
    def _create_document(self, record: Dict, extraction_date: date) -> Dict:
        ...
        return {
            'list_type_id': record.get('listTypeId'),
            'list_type_desc': record.get('listTypeDesc'),
    ...
    def merge_to_core(self) -> Tuple[int, int]:
        ...
        """
        logger.info("Merging trading_code_list from staging to core...")
        ...
        try:
            ...
            for stg_doc in stg_docs:
                ...
                filter_dict = {'list_type_id': doc['list_type_id']}
            ...
            logger.info(f"Merged trading_code_list: {inserted} inserted, {updated} updated")
            return (inserted, updated)
            
        except Exception as e:
            logger.error(f"Error merging trading_code_list to core: {e}")
            raise


if __name__ == "__main__":
    """Standalone run - extract trading code list"""
    extractor = TradingCodeListExtractor()
    
    # Extract trading code list
