"""
Extract expected changes in weight factor from TASE API
...
from mongo_helpers import (
    ...
    parse_date, safe_float, convert_date_to_string
)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("extract_expected_changes_weight_factor.log"),
...
# API configuration
EXPECTED_CHANGES_WEIGHT_FACTOR_ENDPOINT = f"{API_BASE_URL}/indices-parameters-updates/expected-changes-weight-factor"

# MongoDB collection names
STG_COLLECTION = "stg_expected_changes_weight_factor"
CORE_COLLECTION = "core_expected_changes_weight_factor"


class ExpectedChangesWeightFactorExtractor:
    """
    Extract expected changes in weight factor from TASE API
    """
    ...
    def __init__(self):
        ...
        logger.info("Expected Changes Weight Factor: %s", EXPECTED_CHANGES_WEIGHT_FACTOR_ENDPOINT)
        logger.info("Connected to API and MongoDB")

    def _create_indexes(self):
        """Create indexes for better query performance"""
        try:
            self.stg_collection.create_index([("security_id", 1), ("date", 1), ("index_name", 1)], unique=True)
            self.stg_collection.create_index([("security_id", 1)])
            self.stg_collection.create_index([("date", 1)])
            self.stg_collection.create_index([("index_name", 1)])
            
            self.core_collection.create_index([("security_id", 1), ("date", 1), ("index_name", 1)], unique=True)
            self.core_collection.create_index([("security_id", 1)])
            self.core_collection.create_index([("date", 1)])
            self.core_collection.create_index([("index_name", 1)])
    ...
    def extract_for_date(self, trade_date: date) -> Tuple[int, int, int]:
        """
        Extract expected changes weight factor for specific date
        Args:
        ...
        try:
            ...
            url = f"{EXPECTED_CHANGES_WEIGHT_FACTOR_ENDPOINT}/{trade_date.year}/{trade_date.month}/{trade_date.day}"
            ...
            # Extract data
            expected_changes = data.get('expectedChangesWeightFactor', {})
    ...
    def _save_batch(self, records: List[Dict], expected_date: date) -> Tuple[int, int, int]:
        ...
        for record in records:
            try:
                # Extract security_id and index_name
    ...
    def _create_document(self, record: Dict, trade_date: date) -> Dict:
        ...
        return {
            ...
            'previous_factor_date': convert_date_to_string(parse_date(record.get('previousFactorDate'))) if record.get('previousFactorDate') else None,
            'new_weight_factor': safe_float(record.get('newWeightFactor')),
            'weight_factor': safe_float(record.get('weightFactor')),
            'new_issuer_adjusted_weight_cap': safe_float(record.get('newIssuerAdjustedWeightCap'))
        }
    ...
    def merge_to_core(self) -> Tuple[int, int]:
        ...
        """
        logger.info("Merging expected_changes_weight_factor from staging to core...")
        ...
        try:
            ...
            logger.info(f"Merged expected_changes_weight_factor: {inserted} inserted, {updated} updated")

            return (inserted, updated)

        except Exception as e:
            logger.error(f"Error merging expected_changes_weight_factor to core: {e}")
            raise


if __name__ == "__main__":
    """Standalone run - extract yesterday's data"""
    extractor = ExpectedChangesWeightFactorExtractor()
